...
@interface ZXingWidgetController () <UIAlertViewDelegate>
{
...
- (id)initWithDelegate:(id<ZXingDelegate>)scanDelegate showCancel:(BOOL)shouldShowCancel OneDMode:(BOOL)shouldUseoOneDMode showLicense:(BOOL)shouldShowLicense {
    ...
    if (self) {
        ...
        _queue = dispatch_queue_create("com.suning.ZXing", NULL);
...
- (void)goToScanHistory
{
    ...
    wasCancelled = YES;
    if (delegate != nil) {
        [delegate zxingControllerDidTapRightBtn:self];
    }
}
...
- (void)decoder:(ZXING_Decoder *)decoder willDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset{
#if ZXING_DEBUG
    NSLog(@"DecoderViewController MessageWhileDecodingWithDimensions: Decoding image (%.0fx%.0f) ...", image.size.width, image.size.height);
#endif
}

- (void)decoder:(ZXING_Decoder *)decoder
...
- (void)decoder:(ZXING_Decoder *)decoder didDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset withResult:(TwoDDecoderResult *)twoDResult {
    [self presentResultForString:[twoDResult text]];
    [self presentResultPoints:[twoDResult points] forImage:image usingSubset:subset];
    // now, in a selector, call the delegate to give this overlay time to show the points
    [self performSelector:@selector(notifyDelegate:) withObject:[twoDResult retain] afterDelay:0.0];
    decoder.delegate = nil;
}

- (void)notifyDelegate:(TwoDDecoderResult *)ret {
    if (!isStatusBarHidden) [[UIApplication sharedApplication] setStatusBarHidden:NO];
    [delegate zxingController:self didScanResult:ret];
    [ret release];
}

- (void)decoder:(ZXING_Decoder *)decoder failedToDecodeImage:(UIImage *)image usingSubset:(UIImage *)subset reason:(NSString *)reason {
    decoder.delegate = nil;
    [overlayView setPoints:nil];
}

- (void)decoder:(ZXING_Decoder *)decoder foundPossibleResultPoint:(CGPoint)point {
    ...
    if (!captureInput) {
        ...
        if ([[[UIDevice currentDevice] systemVersion] floatValue] >= 7.0)
        {
            UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"系统提示"
                                                            message:@"您已关闭相机使用权限，请至手机“设置->隐私->相机”中打开"
                                                           delegate:self
...
//    NSAutoreleasePool *pool = [[NSAutoreleasePool alloc] init];
    ...
    ZXING_Decoder* d = [[ZXING_Decoder alloc] init];
    d.readers = readers;
    d.delegate = self;
    
    decoding = [d decodeImage:scrn cropRect:[overlayView cropRect]] == YES ? NO : YES;
    ...
    if (decoding) {
        ...
        UIImage *scrn1 = [[UIImage alloc] initWithCGImage:scrn.CGImage
                                           scale:1.0
                                     orientation:UIImageOrientationLeft];
        
        // NSLog(@"^ %@ %f", NSStringFromCGSize([scrn size]), scrn.scale);
        decoding = [d decodeImage:scrn1] == YES ? NO : YES;
